...
class NarrativeItineraries extends Component {
  ...
  _getBaselineCo2 = (mode) => {
    ...
    return coreUtils.itinerary.calculateEmissions(
      { legs: [{ distance: avgDistance, mode }] },
      co2Config?.carbonIntensity,
      co2Config?.massUnit
    )
  }

  _renderItineraryRow = (itinerary, optionNumber, mini = false) => {
    const {
      activeItinerary,
  ...
  render() {
    ...
    const baselineCo2 = this._getBaselineCo2('car')
    const baselineMidCo2 = this._getBaselineCo2('mid')
    const baselineLowCo2 = this._getBaselineCo2('low')
    const itinerariesWithCo2 =
      mergedItineraries?.map((itin) => {
        ...
        return {
          ...
          co2Category: emissions < baselineLowCo2 ? 'low' : emissions > baselineMidCo2 ? 'high' : 'mid',
        }
      }) || []
          ...
          {showingErrors || mergedItineraries.length === 0 ? (
            ...
            <>
              {groupItineraries && !itineraryIsExpanded ? (
                Object.keys(groupedMergedItineraries.multi).map((mode) => {
                  return (
                    <S.ModeResultContainer key={mode}>
                      ...
                      <ListContainer>
                        {groupedMergedItineraries.multi[mode].map((itin, optionNumber) =>
                          this._renderItineraryRow(itin, optionNumber)
                        )}
                      </ListContainer>
                ...
                <ListContainer>
                  {itinerariesWithCo2.map((itin, optionNumber) =>
                    this._renderItineraryRow(itin, optionNumber)
                  )}
                </ListContainer>
              )}
                  ...
                  <S.ModeResultContainer>
                    ...
                    <S.SingleModeRowContainer>
                      {Object.keys(groupedMergedItineraries.single).map(
                        (mode) =>
                          groupedMergedItineraries.single[mode].map((itin, optionNumber) =>
                            this._renderItineraryRow(itin, optionNumber, true)
                          )
                      )}
                    </S.SingleModeRowContainer>
                  </S.ModeResultContainer>
